...
def getType(atom_data, atom_ids, sequences):
...
def compute_degree_of_mixing(op1, op2, density, threshold):
    """based on chain id"""
    # f = np.array([op1[i] / phi[i] if phi[i] >0 else 0 for i in range(len(op1))]) * density_filter(density, threshold)
    # g = np.array([op2[i] / phi[i] if phi[i] >0 else 0 for i in range(len(op2))]) * density_filter(density, threshold)
...
def bin_data(
    ...
    phi_alpha = ref_compositions(
        phase_description["phases"][str(alpha)], phase_description
    )
    phi_beta = ref_compositions(
        phase_description["phases"][str(beta)], phase_description
    )
    ...
    for i in range(len(bins) - 1):
        ...
        if np.linalg.norm(compositions, ord=2) > 0:
            op1 = (
                np.dot(compositions, phi_alpha)
                / np.linalg.norm(compositions, ord=2)
                / np.linalg.norm(phi_alpha, ord=2)
            )
            op2 = (
                np.dot(compositions, phi_beta)
                / np.linalg.norm(compositions, ord=2)
                / np.linalg.norm(phi_beta, ord=2)
            )
        else:
            op1 = op2 = 0
...
if __name__ == "__main__":
    ...
    # In next version, sequence map should be a separate step
    # mol_sequence_map = map_Mol_Sequence(atom_data, phase_description)
