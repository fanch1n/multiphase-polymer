        ...
        for line in f:
            ...
            if 'TIMESTEP' in contents:
                if cnt > 0:
                    ...
                    threshold = max_density/5.
                    left, right = find_condensed_phase_edges(density_profile, bins)

                    assert left < right, "left edge must be smaller than right edge"
                    assert np.mean(density_profile[np.logical_and((left < mid_bins), (mid_bins < right))]) > threshold, \
                        "condensed phase is in the middle"
                    condensed_region = np.logical_and((atom_data[:, -1] > left), (atom_data[:, -1] < right))
                    gas_region = np.logical_or((atom_data[:, -1] < left - 2*Rg), (atom_data[:, -1] > right+2*Rg))
