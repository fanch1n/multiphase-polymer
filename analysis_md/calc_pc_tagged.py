...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tagged", type=str, default=None, help=".p.gz file with the particle tags"
    )
    parser.add_argument(
    ...
    with ExitStack() as stack:
        ...
        for frame in read_traj(clargs.input):
            try:
                ...
                if clargs.tagged is not None:
                    mol_ids = list(mol_sequence_map.keys())
                    for mol in range(int(min(mol_ids)), int(max(mol_ids) + 1)):
                        if mol in mols_in_gas:
                            tag_map[mol] = -1
                        elif mol in mols_in_bulk:
                            tag_map[mol] = 1
                        else:
                            tag_map[mol] = 0

                    pickle.dump(tag_map, foutput)
                    foutput.flush()
            except:
                print(clargs.tagged)
                break
