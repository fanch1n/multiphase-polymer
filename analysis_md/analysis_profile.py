...
def find_condensed_phase_edges(phi, bins):
    ...
    if len(condensed_region) + len(dilute_region) > 3:
        ...
        if len(condensed_region) == 2:
            ...
            assert (region2[-1] + 1) % len(a) <= region1[
                1
            ], f"disconnected condensed region!"
            left = region2[0]
            right = region1[1]
        elif len(condensed_region) == 1:
            left, right = condensed_region[0]
...
def get_shiftProfile(data, bins, target_center_loc=None, style="mid", search_width=5.0):
    ...
    if style == "mid":
        lft = locate_interface(
            phi, bins, left_edge - search_width, left_edge + search_width
        )
        lrt = locate_interface(
            phi, bins, left_edge - search_width, left_edge + search_width
        )
        dshift = target_center_pos - (loc_left + loc_right) / 2.0
    else:
        loc = locate_interface(
            phi, bins, right_edge - search_width, right_edge + search_width
        )
        dshift = target_center_pos - loc

    if target_center_loc is None:
        target_center_loc = (bins[-1] - bins[0]) / 2.0
